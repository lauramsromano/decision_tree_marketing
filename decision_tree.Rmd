...
database: https://archive.ics.uci.edu/dataset/222/bank+marketing | https://www.kaggle.com/datasets/henriqueyamahata/bank-marketing
---
...
#colunas_para_remover <- c("duration")
#base <- base[, !(names(base) %in% colunas_para_remover)]
...
```
``` {R Distribuição - Variáveis categóricas}

colunas_cat <- base %>% 
  select_if(is.factor)
nome_colunas_cat <- colnames(colunas_cat) 
...
for (i in nome_colunas_cat) {
  barplot(table(base[[i]]), main = paste(i), xlab = i, col = "#1f78b4")
}
...
- A idade média das pessoas constatadas é de 40 anos, mas há clientes na faixa dos 80-90 anos. 
- Em média, os cliente foram constatados aproximandamento 3 vezes até realizar sua decisão.
- As ligações duram em médias 300 segundos, 5 minutos.
- A maior parte dos clientes não havia sido contato em campanhias anteriores.
...
Agora, vamos analisar mais profundamento outras questão que podem estar ligadas com a aquisição do produto, como questão econômicas e sociais.
...
p <- ggplot(a, aes(x = month, y = Perc, fill = y)) +
  ...
  geom_text(aes(label = paste0(round(Perc, 1), "%")), 
            position = position_dodge(width = 0.9), vjust = -0.5) +
  labs(title = paste("Percentual de aquisição do produto por Mês"),
       x = i,
       y = "Percentual",
       fill = "target") +
  coord_cartesian(ylim = c(0, max(a$Perc) + 5)) +
...
p <- ggplot(conversion_rates, aes(x = reorder(month, conversion_rate), y = conversion_rate, fill = month)) +
  ...
  geom_text(aes(label = paste0(round(euribor3m, 2))), 
            position = position_dodge(width = 0.9), vjust = -3, color = "#1f78b4") +
  labs(title = "% de Aquisição do Produto x Média Taxa de Juros por Mês",
       x = "Mês",
       y = "Percentual",
       fill = "Mês") +
  coord_cartesian(ylim = c(0, max(conversion_rates$conversion_rate) + 5)) +
...
```
Observamos que o mês de Maio registrou a mais alta taxa de recusa do produto, atingindo 31,3%. Coincidentemente, maio também apresentou a menor taxa de conversão, com apenas 6,4%. Além disso, neste mês, constatamos a quinta maior média de taxa de juros dentro do período analisado.

Por outro lado, março se destacou ao conquistar a maior taxa de conversão, alcançando 50,5%, e ostentou a terceira menor média de taxa de juros, registrando apenas 1,16%.

É possível deduzir que a diminuição das taxas de juros estimulou uma maior adesão ao produto. Isso se explica pelo fato de que, com juros reduzidos, os clientes enfrentam encargos menores, o que torna a aquisição do produto mais vantajosa. Notavelmente, dentre os cinco meses com as taxas de conversão mais elevadas (Março, Dezembro, Setembro, Outubro e Abril), estão também os cinco meses com as menores taxas de juros do período analisado.

```{R Percentual de aquisição do produto por Estado Civíl'}
...
p <- ggplot(a, aes(x = marital, y = Perc, fill = y)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = paste0(round(Perc, 1), "%")), 
            position = position_dodge(width = 0.9), vjust = -0.5) +
  labs(title = paste("Percentual de aquisição do produto por Estado Civil"),
       x = i,
       y = "Percentual",
       fill = "target") +
  coord_cartesian(ylim = c(0, max(a$Perc) + 5)) +
...
``` {R Taxa de conversão por Estado Civíl}
...
p <- ggplot(conversion_rates, aes(x = reorder(marital, conversion_rate), y = conversion_rate, fill = marital)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = paste0(round(conversion_rate * 100, 1), "%")), 
            position = position_dodge(width = 0.9), vjust = -0.5) +
  labs(title = "Percentual de Aquisição do Produto por Estado Civíl",
       x = "Faixa de Idade",
       y = "Percentual",
       fill = "Faixa de Idade") +
  coord_cartesian(ylim = c(0, max(conversion_rates$conversion_rate) + 5)) +
